		...
		unsigned int bitsout, void *din, unsigned int bitsin)
{
	...
	for (count = 0; count < bytesout; count++, dout++) {
		writeb(*(uint8_t *)dout, spibar + 0x0C);
	}
...
{
	uint8_t	cmd_val = 0x96;		/* Kick off IMC Mailbox command 96 */
	uint8_t	reg0_val = 0;		/* clear response register */
	uint8_t	reg1_val = 0xB4;	/* request ownership flag */
...
static void ImcWakeup(void)
{
	uint8_t	cmd_val = 0x96;		/* Kick off IMC Mailbox command 96 */
	uint8_t	reg0_val = 0;;		/* clear response register */
	uint8_t	reg1_val = 0xB5;	/* release ownership flag */
