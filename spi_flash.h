...
struct spi_flash {
	...
	int		(*write)(struct spi_flash *flash, uint32_t offset, size_t len,
			const void *buf);
	int		(*spi_erase)(struct spi_flash *flash, uint32_t offset, size_t len);
	...
	int		(*sec_read)(struct spi_flash *flash, uint32_t offset, size_t len, void *buf);
	int		(*sec_prog)(struct spi_flash *flash, uint32_t offset, size_t len,
			const void *buf);
	int		(*sec_erase)(struct spi_flash *flash, uint32_t offset, size_t len);
	int		(*sec_lock)(struct spi_flash *flash, uint8_t reg);
};
...
static inline int spi_flash_sec_read(struct spi_flash *flash, uint32_t offset, size_t len,
		void *buf)
{
	return flash->sec_read(flash, offset, len, buf);
}

static inline int spi_flash_sec_prog(struct spi_flash *flash, uint32_t offset, size_t len,
		const void *buf)
{
	return flash->sec_prog(flash, offset, len, buf);
}

static inline int spi_flash_sec_erase(struct spi_flash *flash, uint32_t offset, size_t len)
{
	return flash->sec_erase(flash, offset, len);
}

static inline int spi_flash_sec_lock(struct spi_flash *flash, uint8_t reg)
{
