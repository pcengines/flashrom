...
static int winbond_write(struct spi_flash *flash, uint32_t offset, size_t len, const void *buf)
{
...
static int winbond_erase(struct spi_flash *flash, uint32_t offset, size_t len)
{
...
static int winbond_sec_read(struct spi_flash *flash, uint32_t offset, size_t len, void *buf)
{
	int ret = 1;
	uint8_t cmd[5];
	uint8_t reg = (offset >> 8) & 0xFF;
...
static int winbond_sec_program(struct spi_flash *flash, uint32_t offset, size_t len, const void *buf)
{
	int ret = 1;
	uint8_t cmd[4];
	uint8_t reg = (offset >> 8) & 0xFF;
	uint8_t addr = offset & 0xFF;
	uint32_t tmp_sect_size = flash->sector_size;
...
static int winbond_sec_erase(struct spi_flash *flash, uint32_t offset, size_t len)
{
	int ret = 1;
	uint8_t reg = (offset >> 8) & 0xFF;
	uint32_t tmp_sect_size = flash->sector_size;
...
static int winbond_sec_lock(struct spi_flash *flash, uint8_t reg)
{
	int ret;
	uint8_t cmd;
	uint8_t status[2];
